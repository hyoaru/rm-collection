...
export default async function createMayaCheckout({ orders }: CreateMayaCheckoutParams) {
  ...
  if (!["pending", "failure", "cancel"].includes(orderBillingTransaction?.status!)) {
    response.error = {
    ...
    const isMayaCheckoutSessionExpired = dayjs() > dayjs(billingTransactionDateLastUpdated).add(30, "minute");
