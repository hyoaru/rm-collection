...
export default function BarChartSalesThroughoutYears(props) {
  const [year, setYear] = useState(dayjs().format('YYYY'))
  const { orders } = props

  function getOrdersGroupedByYearMonth(data) {
    const ordersGroupedByYearMonth = {}
    let yearList = []

    const ordersGroupedByYear = Object.groupBy(data, ({ created_at }) => dayjs(created_at).year())
    yearList = Object.keys(ordersGroupedByYear)

    Object.entries(ordersGroupedByYear).forEach(([year, recordsByYear]) => {
      const recordsGroupedByMonth = Object.groupBy(recordsByYear, ({ created_at }) => dayjs(created_at).month())
      ...
      <div className="relative">
        <div className="absolute top-0 right-0 left-0 z-[5]">
          <div className="flex ">
            ...
            <div className="">
              <Select className={''} value={year} onValueChange={setYear}>
                <SelectTrigger>
