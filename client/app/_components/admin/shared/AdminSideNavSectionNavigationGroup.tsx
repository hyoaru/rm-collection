  ...
  return (
    <>
      <div className="border border-x-0 py-5 px-2 lg:px-5 pb-8 flex flex-row overflow-x-auto rounded-lg gap-2 sm:flex-col">
        <small className="text-center uppercase mb-1 text-muted-foreground hidden sm:block">{sectionTitle}</small>
        {navigations.map((navigation) => {
          ...
          if (isPermitted) {
            return (
              <Link key={`SideNavExpandedNavigation-${navigation.name}`} href={navigation.pathName}>
